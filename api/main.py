...
try:
    ...
    print(f"Error loading model: {e}.assurez vous que le modèle a bien été entraîné en exécutant 'python -m src.model'.")
    model = None
except Exception as e:
    print(f"erreur inattendue est survenue lors du chargement du modèle: {e}")
    model = None

app = FastAPI(
    title="House Price Prediction API",
    description="API pour prédire les prix des maisons en fonction de différentes caractéristiques.",
    version="1.0.0",
)
...
class HouseFeatures(BaseModel):
    ...
    date: str = Field(..., example="2025-01-15", description="Date de vente (AAAA-MM-JJ)")
...
async def predict_price(features: HouseFeatures):
    if model is None:
        raise HTTPException(status_code=500, detail="Modèle non chargé. Veuillez contacter l’administrateur.")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"La prédiction a échoué en raison dune erreur interne: {e}")